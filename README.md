...
### Was macht der Mod?
LazyChilla AD Extension fügt dem AutoDrive-HUD eine frei belegbare Schnellzugriff-Leiste hinzu. Ein fester **Werkstatt-Button** (ganz rechts) plus beliebig viele **dynamische Buttons** — jeweils mit einem Icon aus 27 verfügbaren Icons belegt. Ziele werden mit dem gleichen Mechanismus gesetzt wie der Parkplatz in AutoDrive.

### Features
- **Werkstatt-Button** fest rechts — immer verfügbar, ein Klick zur Hofwerkstatt
...
- **Persistent gespeichert** (modSettings) — Buttons und Ziele bleiben nach Neustart erhalten
...
| Werkstatt (Tastenbelegung) | `STRG + ALT + W` (frei änderbar in den Spieleinstellungen) |
...
### Mitwirken
Bug gefunden oder Idee für ein Feature? Gerne ein [Issue](../../issues) eröffnen oder einen Pull Request einreichen.
...
LazyChilla AD Extension adds a customizable quick-access button bar to the AutoDrive HUD. One fixed **Workshop button** (far right) plus as many **dynamic buttons** as you like — each assigned one of 27 available icons. Destinations are set using the same mechanism as the AutoDrive parking spot.

### Features
- **Workshop button** fixed on the right — always available, one click to the farm workshop
...
- **Persistent storage** (modSettings) — buttons and destinations survive game restarts
...
| Workshop (keybinding) | `CTRL + ALT + W` (freely remappable in game settings) |
...
### Contributing
Found a bug or have a feature idea? Feel free to open an [issue](../../issues) or submit a pull request.
...
### Que fait ce mod ?
LazyChilla AD Extension ajoute une barre de raccourcis personnalisable au HUD d'AutoDrive. Un **bouton Atelier** fixe (tout à droite) plus autant de **boutons dynamiques** que vous voulez — chacun associé à une icône parmi 27 disponibles. Les destinations se définissent avec le même mécanisme que la place de parking AutoDrive.

### Fonctionnalités
- **Bouton Atelier** fixe à droite — toujours disponible, un clic vers l'atelier
...
- **Sauvegarde persistante** (modSettings) — boutons et destinations conservés après redémarrage
...
| Atelier (raccourci) | `CTRL + ALT + W` (modifiable dans les paramètres du jeu) |
...
### Contribuer
Bug trouvé ou idée de fonctionnalité ? N'hésitez pas à ouvrir une [issue](../../issues) ou soumettre une pull request.
...
### Cosa fa questa mod?
LazyChilla AD Extension aggiunge una barra di accesso rapido personalizzabile all'HUD di AutoDrive. Un **pulsante Officina** fisso (tutto a destra) più tutti i **pulsanti dinamici** che vuoi — ognuno abbinato a un'icona tra 27 disponibili. Le destinazioni si impostano con lo stesso meccanismo del parcheggio AutoDrive.

### Caratteristiche
- **Pulsante Officina** fisso a destra — sempre disponibile, un click verso l'officina
...
- **Salvataggio persistente** (modSettings) — pulsanti e destinazioni conservati dopo il riavvio
...
| Officina (tasto) | `CTRL + ALT + W` (modificabile nelle impostazioni di gioco) |
...
### Contribuire
Hai trovato un bug o hai un'idea? Apri pure una [issue](../../issues) o invia una pull request.
...
### O que este mod faz?
LazyChilla AD Extension adiciona uma barra de acesso rápido personalizável ao HUD do AutoDrive. Um **botão Oficina** fixo (extrema direita) mais quantos **botões dinâmicos** quiser — cada um com um ícone entre 27 disponíveis. Os destinos são definidos com o mesmo mecanismo do estacionamento do AutoDrive.

### Funcionalidades
- **Botão Oficina** fixo à direita — sempre disponível, um clique para a oficina
...
- **Armazenamento persistente** (modSettings) — botões e destinos mantidos após reinicialização
...
| Oficina (tecla) | `CTRL + ALT + W` (alterável nas configurações do jogo) |
...
### Contribuir
Encontrou um bug ou tem uma ideia? Fique à vontade para abrir uma [issue](../../issues) ou enviar um pull request.

---

## License / Lizenz / Licence / Licenza / Licença
